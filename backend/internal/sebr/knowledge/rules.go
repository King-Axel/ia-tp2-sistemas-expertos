...
func Rules() []domain.Rule {
	return []domain.Rule{
		...
		rule("R05", "Confirma cuadro respiratorio", domain.All(domain.BoolFact(suspectedFeverCase, true), domain.BoolFact(suspectedRespiratoryCase, true)), respiratoryCase, defaultPriority, "sospecha_cuadro_febril AND sospecha_cuadro_respiratorio"),
		...
		rule("R14", "Sospecha COVID a partir de caso evaluable y contacto estrecho", domain.All(domain.BoolFact(evaluableCase, true), domain.BoolFact(closeContact, true)), suspectedCovid, defaultPriority, "caso_evaluable AND contacto_estrecho == true"),
		rule("R15", "Compatibilidad con COVID a partir de caso evaluable y pérdida de gusto y olfato", domain.All(domain.BoolFact(evaluableCase, true), domain.BoolFact(lossOfTasteOrSmell, true)), covidCompatible, defaultPriority, "caso_evaluable AND perdida_gusto_u_olfato == true"),
